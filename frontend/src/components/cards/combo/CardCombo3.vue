	...
	<n-card content-style="padding:0" hoverable>
		<div class="card-wrap flex flex-col">
			<div class="overlay" :class="{ twoSeries }">
				<div class="info">
					<div class="title">
						<span>Registration Chart</span>
					</div>

					<!-- <div class="box-wrapper flex">
						<CardCombo4 title="Leads" valString="248.3K" percentage :percentageProps="{
							value: 2.45,
							direction: 'up'
						}" />
						<CardCombo4 v-if="twoSeries" title="Sales" valString="$37.5K" percentage :percentageProps="{
							value: 1.96,
							direction: 'down'
						}" />
					</div> -->
				...
				<div class="toolbar flex gap-2">
					<div v-if="twoSeries" class="flex gap-2">
						<n-button v-for="series of chartSeries" :key="series.name" @click="toggleSeries(series)" secondary
			...
			<n-spin :show="!loaded || loading" class="chart grow">
				<DemoChart v-if="loaded" type="area" :seriesList="twoSeries ? ['Leads', 'Clicks', 'Sales'] : ['Users']"
					colorsSecondary :dataType="chartTypeValue" :strokeWidth="2" hideLegend :legendOffset="130"
					:fontColor="textSecondaryColor" @mounted="setChartContext" :series="chartData.series"
...
const { oneSeries, chartData, loading } = toRefs(props)
