<template>
	<n-scrollbar class="notifications-list">
		<div class="item flex" v-for="item of listSanitized" :key="item.id" @click="item.action ? item.action() : () => { }"
			...
			<div class="content grow">
				...
				<div class="date">{{ toRelativeTime(item.created_at) }}</div>
			</div>
...
const list = useNotifications().notifications
