<template>
	<nav class="nav" :class="[{ collapsed }, mode]">
		<n-menu ref="menu" :options="menuOptions" :collapsed="collapsed" :mode="mode" :accordion="false"
			:collapsed-width="collapsedWidth" :dropdown-props="{
				scrollable: true,
				menuProps: () => ({
					class: 'main-nav'
				})
			}" v-model:value="selectedKey" :expanded-keys="expandedKeys" @update:expanded-keys="handleUpdateExpandedKeys" />
	</nav>
</template>
