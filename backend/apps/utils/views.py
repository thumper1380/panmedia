...
class DrillDownAPIView(APIView):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ignore_fields = set(
            ['fields', 'limit', 'offset', 'format', 'order_by'] + self.ignore + self.hide_fields)
    ...
    def create_nested_json(self, data, drilldowns):
        if not drilldowns:
            return data
        ...
        for key, value in grouped_data.items():
            grouped_data[key] = self.create_nested_json(value, drilldowns[1:])
    ...
    def get(self, request, *args, **kwargs):
        ...
        # fields = self.request.QUERY_PARAMS.get('fields', None) # this line is deprecated in DRF 3.0
        fields = self.request.query_params.get(
            'fields', None)  # use this instead
        # fields parameter is a comma-delimited list
        ...
        for f in request_params:
            # split so you catch things like "invoice.total__lt=10000"
            if f.split('__')[0] not in self.ignore_fields:
        ...
        print('Drilldowns: ', self.drilldowns)
        if self.error:
        ...
        # self.offset = int_or_none(self.request.QUERY_PARAMS.get('offset')) or 0 # this line is deprecated in DRF 3.0
        self.offset = int_or_none(self.request.query_params.get(
            'offset')) or 0  # use this instead

        # self.limit = int_or_none(self.request.QUERY_PARAMS.get('limit')) or 0 # this line is deprecated in DRF 3.0
        self.limit = int_or_none(self.request.query_params.get(
            'limit')) or 0  # use this instead
        ...
        # return the response
        try:
            data = self.create_nested_json(serializer.data, self.drilldowns)
        except FieldError:
            # typical error
            return _error('Error: May be bad field name in order_by')
    ...
    def _validate_drilldowns(self, drilldowns):
        ...
        def validate_me(current_model, dd_string, current_string=''):
            ...
            if not new_model:
                self.error = ('%s: "%s" is not a ForeignKey, ManyToMany, OneToOne, ManyToOneRel, or OneToOneRel.'
    ...
    def _create_fields_map(self, fields):
        ...
        def add_to_fields_map(current_model, current_map, dot_string, current_related=''):
            ...
            # ALL is allowed in fields_map
            if not (fieldname == 'ALL' or is_field_in(current_model, fieldname)):
                self.error = ('%s: "%s" is not a valid field' %
                              (ERROR_STRING, dot_string))
                return None
            ...
            if fieldname == 'ALL':
                ...
                for field in current_model._meta.get_fields():
                    ...
                    add_to_fields_map(
                        current_model, current_map, dot_string=fname, current_related=current_related)
                ...
                if new_model and (field_type == ManyToManyField or there_are_subfields):
                    ...
                    current_related = (current_related +
                                       '__' + fieldname).strip('__')
                    if current_related in self.drilldowns:
                        field_type = get_field_type(current_model, fieldname)
                        if field_type in [ForeignKey, OneToOneField, ManyToOneRel, OneToOneRel, CharField]:
                            self.select_relateds.append(current_related)
                        else:
                            self.prefetch_relateds.append(current_related)
                    else:
                        self.error = ('%s: %s is not valid' % (
                            ERROR_STRING, current_related.replace('__', '.')))
                        return None

                    # Add sub-field to fields_map
                    if there_are_subfields:
                        add_to_fields_map(
                            new_model, current_map, pair[1], current_related)  # recurse
                    else:
                        # defaults to return the id only
                        add_to_fields_map(new_model, current_map, 'id')
                elif there_are_subfields:  # requested a sub-field for a field that's not a model, e.g. amount.profile
                    self.error = ('%s: %s not valid field' %
                                  (ERROR_STRING, dot_string))
                    return None
    ...
    def _set_relateds(self, fields_map):
        ...
        def add_to_relateds(current_model, current_map, fieldname, current_string=''):
            ...
            if current_map[fieldname]:  # e.g. if there are sub-fields
                field_type = get_field_type(current_model, fieldname)
                current_string = (current_string + '__' +
                                  fieldname).strip('__')
                if field_type in [ForeignKey, OneToOneField, ManyToOneRel, OneToOneRel, ManyToManyField]:
                    if not current_string in self.drilldowns:
                        self.error = ('Error: %s not valid' %
                                      current_string.replace('__', '.'))
                        return None
                    if field_type in [ForeignKey, OneToOneField, ManyToOneRel, OneToOneRel]:
                ...
                for f in current_map.get(fieldname, {}):
                    add_to_relateds(
                        new_model, current_map[fieldname], f, current_string)  # recursion
    ...
    def _set_filter_kwargs(self, filters):
        ...
        for p in filters:
            ...
            dot_string = pair[0]
            if len(pair) > 1:
                # 'operation' is something like '__gt', '__isnull', etc.
                operation = '__' + pair[1]
            else:
                operation = ''
            ...
            def do_filter(dot_string, filter_string, current_model):
                ...
                if not is_field_in(current_model, fieldname):
                    if self.picky:
                        ...
                        self.warning += '"%s" is not a valid parameter.  ' % filter_string.replace(
                            '__', '.')
                    return None
                ...
                if leftover:
                    ...
                    if filter_string not in self.drilldowns:
                        if self.picky:
                            self.error = 'Error in filters: %s' % filter_string.replace(
                                '__', '.')
                        else:
                            self.warning += '"%s" is not a valid parameter.  ' % filter_string.replace(
                                '__', '.')
                        return None
                    if field_type not in [ForeignKey, OneToOneField, ManyToOneRel, OneToOneRel, ManyToManyField]:
                        if self.picky:
                            self.error = (
                                'Error: %s has no children' % filter_string)
                        else:
                            self.warning += '"%s" is not a valid parameter.  ' % filter_string.replace(
                                '__', '.')
                        return None
                    ...
                    # recursion
                    return do_filter(leftover, filter_string, current_model)
                else:
            ...
            if filter_string:
                ...
                if operation in ["__in"]:
                    # comma_separated_multiple_values = self.request.QUERY_PARAMS[p] # this line is deprecated in DRF 3.0
                    # use this instead
                    comma_separated_multiple_values = self.request.query_params[p]
                    filter_kwargs[filter_string +
                                  operation] = comma_separated_multiple_values.split(",")
                else:
                    # filter_kwargs[filter_string + operation] = self.request.QUERY_PARAMS[p] # this line is deprecated in DRF 3.0
                    # use this instead
                    filter_kwargs[filter_string +
                                  operation] = self.request.query_params[p]
        ...
        def __init__(self, *args, **kwargs):
            ...
            if fields_map:
                ...
                def prune_fields(fields_map, model):
                    ...
                    # flatten to get fields requested for this specific serializer model
                    requested = list(fields_map)
                    # by default this is all fields for the model
                    available = list(self.fields)
                    for field_name in set(available) - set(requested):
                        # delete the ones we don't want from the serializer
                    ...
                    for field_name in fields_map:
                        ...
                        # only do this for fields with sub-fields requested
                        if sub_fm and sub_fm != {'id': {}}:
                            ftype = get_field_type(model, field_name)
                            if ftype in [ForeignKey, OneToOneField, ManyToOneRel, OneToOneRel, ManyToManyField, CharField]:
...
def get_model(parent_model, fieldname):
    ...
    field_class = parent_model._meta.get_field(fieldname)
    field_type = type(field_class)
    if field_type in [ForeignKey, ManyToManyField, OneToOneField]:
        model = parent_model._meta.get_field(fieldname).remote_field.model
    elif field_type == ManyToOneRel:
        model = parent_model._meta.get_field(fieldname).related_model
    elif field_type == OneToOneRel:
        model = field_class.related_model
...
#     fieldnames = model._meta.get_fields()
#     return fieldname in fieldnames

def is_field_in(model, fieldname):
    """Return true if fieldname is a field or related object in model"""
...
# from django.conf import settings
# from django.db import connection
# from django.db.models.fields.related import ForeignKey, OneToOneField, ManyToManyField, ManyToOneRel, OneToOneRel
# from django.core.exceptions import FieldError
# from rest_framework import serializers
# from rest_framework.response import Response
# from rest_framework.views import APIView
...
#                 # add in all the fields for the model
#                 fname_prefix = current_related.replace('__', '.') + '.'
#                 for fname in [field.name for field in current_model._meta.get_fields()]:
#                     if (fname_prefix + fname).strip('.') in self.hide_fields:
#                         continue  # skip it
#                     field_type = get_field_type(current_model, fname)
#                     # don't add the field if it's a related field and out of drilldowns range
...
#                             ftype = get_field_type(model, field_name)
#                             if ftype in [ForeignKey, OneToOneField, ManyToOneRel, OneToOneRel, ManyToManyField]:
#                                 m = get_model(model, field_name)
#                                 self.fields[field_name] = DrilldownSerializerFactory(m)(
#                                     fields_map=fields_map[field_name])  # recursively create another serializer
...
#     #     model = parent_model._meta.get_field(fieldname).remote_field
#     else:
#         model = None
#     return model
...
# # def get_model(parent_model, fieldname):
# #     """Get the model of a foreignkey, manytomany, etc. field"""
# #     field = parent_model._meta.get_field(fieldname)
...
# def is_field_in(model, fieldname):
#     """Return true if fieldname is a field or relatedobject in model"""
#     fieldnames = [field.name for field in model._meta.get_fields()]
#     return fieldname in fieldnames
