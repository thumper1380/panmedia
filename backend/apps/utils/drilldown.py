...
class DrillDownAPIView(APIView):
    ...
    def get(self, request):
        ...
        serializer = DrilldownSerializerFactory(self.model)(
            fields_map=self.fields_map, instance=qs, many=True)
...
def get_model(parent_model, fieldname):
    ...
    if field_type in [ForeignKey, ManyToManyField, OneToOneField]:
        model = field_class.remote_field.model
...
def is_field_in(model, fieldname):
    ...
    return any(field.name == fieldname for field in model._meta.get_fields())
